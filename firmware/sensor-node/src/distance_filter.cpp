...
// Mảng tĩnh kích thước FILTER_HISTORY_SIZE, sort/min/max viết tay
...
// Insertion sort tại chỗ - đủ nhanh vì count <= FILTER_HISTORY_SIZE (rất nhỏ).
...
{
    float dynamicTolerance = centerCm * FILTER_CLUSTER_TOLERANCE_RATIO;
    return dynamicTolerance < FILTER_BASE_CLUSTER_TOLERANCE_CM
               ? FILTER_BASE_CLUSTER_TOLERANCE_CM
               : dynamicTolerance;
}

float getJumpThreshold(float currentDistanceCm)
{
    float dynamicThreshold = currentDistanceCm * FILTER_JUMP_THRESHOLD_RATIO;
    return dynamicThreshold < FILTER_MIN_JUMP_THRESHOLD_CM
               ? FILTER_MIN_JUMP_THRESHOLD_CM
               : dynamicThreshold;
}

float getJumpTolerance(float candidateCm)
{
    float dynamicTolerance = candidateCm * FILTER_JUMP_TOLERANCE_RATIO;
    return dynamicTolerance < FILTER_BASE_JUMP_TOLERANCE_CM
               ? FILTER_BASE_JUMP_TOLERANCE_CM
               : dynamicTolerance;
}
...
struct ClusterSearchResult
{
    bool found;
    float center;
    float members[FILTER_HISTORY_SIZE];
    size_t memberCount;
    float spread;
};

// Tìm cụm mạnh nhất trong lịch sử RAW (toàn bộ mảng trung gian là mảng
// tĩnh, history luôn <= FILTER_HISTORY_SIZE nên không bao giờ tràn).
                                     ...
                                     bool hasReference, float referenceCm)
{
    ...
    if (count < (size_t)FILTER_MIN_SAMPLES)
    {
        return best;
    }

    float bestMembers[FILTER_HISTORY_SIZE];
    ...
    for (size_t c = 0; c < count; ++c)
    {
        ...
        float sortedMembers[FILTER_HISTORY_SIZE];
    ...
    if (bestMemberCount < (size_t)FILTER_MIN_CLUSTER_SIZE)
    {
        best.found = false;
        return best;
    }
...
{
    if (_historyCount < FILTER_HISTORY_SIZE)
    {
        _rawHistory[_historyCount++] = distanceCm;
        return;
    }

    for (size_t i = 1; i < FILTER_HISTORY_SIZE; ++i)
    {
        _rawHistory[i - 1] = _rawHistory[i];
    }
    _rawHistory[FILTER_HISTORY_SIZE - 1] = distanceCm;
}

// values luôn là tập con của _rawHistory nên count <= FILTER_HISTORY_SIZE.
void DistanceFilter::replaceHistoryWith(const float *values, size_t count)
{
    size_t copyCount = count > FILTER_HISTORY_SIZE ? FILTER_HISTORY_SIZE : count;
...
bool DistanceFilter::updateJumpCandidate(float newDistanceCm)
{
    ...
    return _jumpCandidateCount >= FILTER_JUMP_CONFIRM_COUNT;
}
...
FilterResult DistanceFilter::process(float rawDistanceCm)
{
    ...
    if (differenceCm <= jumpThresholdCm)
    {
        ...
        _stableDistanceCm =
            FILTER_EMA_ALPHA * cluster.center + (1.0f - FILTER_EMA_ALPHA) * _stableDistanceCm;
