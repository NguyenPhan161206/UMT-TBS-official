 ...
 *
 * B4 scaffold: LVGL UI + sensor_model + coreiot_client (MQTT path).
 * B5 sẽ nối ESP-NOW receiver qua firmware/shared/espnow_protocol.h
 * (đường chính độ trễ thấp); MQTT/CoreIoT là đường phụ.
 */
...
static void on_mqtt_status(bool is_connected)
{
    ...
    /* is_connected đã phản ánh trong on_wifi_status; MQTT chỉ bổ sung trạng thái
     * khi Wi-Fi đã up: cập nhật badge MQTT riêng qua set_iot_status(false/ip?). */
...
void app_main(void)
{
    ...
    /* Đường phụ: CoreIoT MQTT (Wi-Fi STA). ESP-NOW receiver (đường chính)
     * được nối trong bước B5. */
