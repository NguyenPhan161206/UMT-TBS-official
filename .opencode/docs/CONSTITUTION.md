...
>
> Mức vi phạm: **P0** (R1, R2, R3, R4, R8, R9) = chặn commit, sửa ngay trong cùng PR. **P1**
> (R5, R6, R7, R10, R11, R12) = cảnh báo, có kế hoạch sửa trong sprint. Sau mỗi task báo DONE,
> gọi skill `selfcheck` (10 câu) trước khi kết luận.

---

## R1 — Secrets tuyệt đối không vào file tracked 🔴
- **Luật**: credential (token CoreIoT, Wi-Fi pass, API key) **chỉ nằm ở** `config/keys.json`
  (gitignored) hoặc file sinh từ nó (vd `credentials.h`, gitignored, sinh bởi
...
- **Hành động**: sửa ngay, xoá khỏi staged/commit, báo ledger.
...
- **Luật**: demo/example/auto-play nằm ở `prototypes/`, nhánh riêng, hoặc sau macro rõ ràng;
  không có task mô phỏng trong luồng cảnh báo chính.
- **Kiểm tra**: review code (arch-guard).
...
- **Luật**: chỉ `origin` (repo UMT-TBS-official); commit message conventional
  ...
  cấm force push.
- **Kiểm tra**: `git remote -v` = 1 remote; plugin guard + permission.
...
- **Kiểm tra**: số lượng agent trong `.opencode/agent/` không phình vô cớ.
