...
| 4 | T1.4 record_telemetry.py + replay_telemetry.py | DONE | record connect MQTT rc=0; replay dry-run 0/8 dòng exit 0; pytest 29 | record cần board+token để có dữ liệu thật — script/schema/publish verify xong; xong 2026-09-10 (chưa merge main) |
...
- **T1.4 (DONE, step 4):** `record_telemetry.py` subscribe topic telemetry → JSONL (dòng = payload V2 + `recv_epoch_ms`), CLI `--seconds/--out/--topic/--broker/--port/--token`; `replay_telemetry.py` đọc JSONL, validate schema V2 (thiếu key / distance âm → exit 1), publish đúng nhịp (`recv_epoch_ms` ưu tiên, fallback `timestamp`), CLI `--in/--dry-run/--topic/--scale`; R1 token từ keys.json/env/--token, mask khi in.
...
## Deviations
- **2026-09-10 — Step 4 (T1.4 record/replay) DONE, nhánh nguyen:** tạo `tools/record_telemetry.py` + `tools/replay_telemetry.py` + 4 tests mới (pytest 29). Deviation so với prompt: thêm `--broker/--port` ở cả 2 script; `recv_epoch_ms` (thời điểm host nhận) → chuẩn cho T5.7 latency; replay bỏ field này khi republish để payload giữ đúng V2. Nửa record cần board flash `yolo_uno_coreiot` + token thật — verify record đạt connect rc=0 + file rỗng hợp lệ; dry-run + fixture bad/good đủ chứng minh schema/publish path. Verify: record 0 dòng, replay 0/8 dòng exit 0, bad schema exit 1, pytest 29, arch/scan/rulechain OK. Log: `docs/logs/TOOLS_RECORD_REPLAY_LOG.md`. Sẽ push origin/nguyen.
- **2026-09-10 — Step 3 (T1.2 host_sim LVGL SDL) DONE, nhánh nguyen:** tạo `host_sim/umt_dash_sim` (LVGL v9.1.0 FetchContent + SDL2) compile file UI THẬT qua stub mỏng. Deviation so với prompt: (1) local không có xvfb-run/sudo → verify bằng `SDL_VIDEODRIVER=dummy` (tương đương headless, exit 0 + ctest 2/2), CI chạy xvfb-run đúng DoD; (2) `ui_dashboard.h` trong target_files không đổi vì đã là contract đúng — không cần sửa; (3) LV_COLOR_DEPTH 16 (RGB565 khớp màn hình). Verify: ctest 2/2, scenario approach 8/8 mốc exit 0, replay 2 dòng OK, pio sensor-node OK, pytest 25, arch_guard/scan_secrets OK. Log: `docs/logs/HOST_SIM_LOG.md`. ĐÃ commit `b80b876` + push origin/nguyen.
